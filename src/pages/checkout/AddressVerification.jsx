...
const AddressVerification = () => {
  ...
  }, [user?.address, user?.phNumber, user?.id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        createApiUrl(API_ENDPOINTS.USER_DATA(user.id)), 
        payload,
        {
  ...
  const isFormValid = form.address.trim() && form.phNumber.trim();
